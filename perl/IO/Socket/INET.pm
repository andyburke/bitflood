...
sub configure {
	...
	$sock->socket(AF_INET, $type, $proto) or
	  ...
	  my $result = $sock->blocking($arg->{Blocking});
	  if ($^O eq 'MSWin32') {
...
#        my $before = time() if $timeout;
	...
	undef $@;
	my $result = $sock->connect(pack_sockaddr_in($rport, $raddr));
	# non-blocking connect() always returns false on MSWin32, even if valid
        if ($result || ($^O eq 'MSWin32' && !$sock->blocking)) {
